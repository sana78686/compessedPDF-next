...
function normalizeBlogs(res: unknown): { id: number; title: string; slug: string; excerpt?: string }[] {
  if (Array.isArray(res)) return res as { id: number; title: string; slug: string; excerpt?: string }[]
  if (res && typeof res === 'object') {
    ...
    const raw = Array.isArray(o.blogs)
      ...
      : Array.isArray(o.data)
        ? o.data
        : []
    return raw as { id: number; title: string; slug: string; excerpt?: string }[]
  }
  return []
}
  ...
  return (
    <div className="blog-list-page wrap">
      ...
      <ul className="blog-list">
        {blogs.map((b) => (
          <li key={b.id}>
            <Link href={`/blog/${encodeURIComponent(b.slug)}`}>{b.title}</Link>
          </li>
        ))}
      </ul>
    </div>
